...
1. Present findings with clear severity levels (Critical/High/Medium/Low)
2. Include actionable remediation steps for each finding
3. Base findings on actual code inspection and command output
4. Prioritize exploitable vulnerabilities over theoretical issues
5. Format output as a structured security report with clear sections
...
- Run: `grep -r "password|secret|key|token" . --exclude-dir=node_modules --exclude-dir=.git | grep -v ".toml" | head -10`

### 2. Dependency Security
- Check for known vulnerabilities: Run `npm audit --audit-level=moderate 2>/dev/null || echo "No package.json found or npm not available"`
...
- Examine recent commits for security implications: Run `git log --oneline -10`
