...
function M.set(namespace, name, opts)
  ...
  lk.wrap_err(fmt("failed to set %s because", name), api.nvim_set_hl, namespace, name, hl)
end
...
function M.win_hl.adopt(win_id, target, name, fallback)
  ...
  local hl = lk.find(function(part)
...
function M.all(hls, namespace)
  lk.foreach(function(hl)
    M.set(namespace or 0, next(hl))
  end, hls)
end

----------------------------------------------------------------------
-- NOTE: Plugin highlights {{{
----------------------------------------------------------------------
...
function M.plugin(name, opts)
  ...
  M.all(opts)
  lk.augroup(fmt("%sHighlightOverrides", name), {
    {
      event = { "ColorScheme" },
      command = function()
