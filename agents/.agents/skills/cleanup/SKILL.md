...
1. Present findings with priority levels (High/Medium/Low impact)
2. Include safety assessment and risk level for each cleanup
3. Distinguish between automated and manual cleanup opportunities
4. Provide specific commands and verification steps
...
- Check package.json for unused dependencies: Run `test -f package.json && npm ls --depth=0 2>/dev/null | grep -E "extraneous|UNMET" || echo "No package.json found"`
- Identify outdated packages: Run `npm outdated 2>/dev/null || echo "npm not available"`
...
- Find empty files and directories: Run `find . -type d -empty -not -path "./.git/*" 2>/dev/null | head -10`
- Identify temporary files and build artifacts
- Look for old backup files and unused assets
- Check for large files that could be optimized: Run `find . -type f -size +1M -not -path "./.git/*" -not -path "./node_modules/*" 2>/dev/null | head -10`
...
- Check for large files in git history: Run `git rev-list --objects --all | git cat-file --batch-check='%(objecttype) %(objectname) %(objectsize) %(rest)' | sed -n 's/^blob //p' | sort --numeric-sort --key=2 | tail -10`
- Look for merged branches that can be deleted: Run `git branch --merged | grep -v "main|master|develop" | head -10`
- Identify stale branches: Run `git for-each-ref --format='%(refname:short) %(committerdate:relative)' refs/heads | sort -k2 | head -10`
