...
local luarocks_success = pcall(require, 'luarocks.loader')
...
log.info("Package Path   : %s", package.path)
log.info("LuaRocks Loaded: %s", luarocks_success)

-- TODO: Figure out environment variables for more portable config
-- local submodule_dir = os.getenv("XDG_CONFIG_HOME") .. '/submodules/'
local submodule_dir = '/home/lalitmee/.config/submodules/'
...
local theme_ok = pcall(
  function()
    local powerarrow_theme = submodule_dir .. 'powerarrow/themes/powerarrow/theme.lua'
...
if not theme_ok then beautiful.init(gears.filesystem.get_themes_dir() .. 'default/theme.lua') end
...
mylauncher = awful.widget.launcher(
  {image = beautiful.awesome_icon, menu = my_main_menu}
)
...
local taglist_buttons = gears.table.join(
  ...
  awful.button({mods.super}, 1, function(t) if client.focus then client.focus:move_to_tag(t) end end),
  awful.button({}, 3, awful.tag.viewtoggle),
  awful.button({mods.super}, 3, function(t) if client.focus then client.focus:toggle_tag(t) end end),
...
root.buttons(
  gears.table.join(
    awful.button({}, 3, function() my_main_menu:toggle() end), awful.button({}, 4, awful.tag.viewnext),
...
local globalkeys = gears.table.join(
  ...
  awful.key(
    ...
    {mods.super}, 'w', function() my_main_menu:show() end, {
  ...
  awful.key(
    {mods.super}, 'Return', function() awful.spawn(terminal) end, {
...
clientkeys = gears.table.join(
  awful.key(
    ...
    {mods.super, mods.shift}, 'c', function(c) c:kill() end, {
      description = 'close',
      group = 'client',
    }
  ), awful.key(
...
client.connect_signal(
  'mouse::enter', function(c) c:emit_signal(
    'request::activate', 'mouse_enter', {raise = false}
  ) end
)

client.connect_signal('focus', function(c) c.border_color = beautiful.border_focus or '' end)
