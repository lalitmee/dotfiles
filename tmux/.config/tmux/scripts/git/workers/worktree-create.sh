...
if [ -z "$REPO_ROOT" ] || [ -z "$WORKTREE_DIR" ] || [ -z "$BRANCH_NAME" ]; then
    ...
    echo "Usage: worktree-create.sh <repo_root> <worktree_dir> <branch_name> [base_branch]"
...
if [ -n "$BASE_BRANCH" ]; then
    ...
    if gum spin --spinner dot --title "Creating worktree for $BRANCH_NAME..." --show-output -- \
