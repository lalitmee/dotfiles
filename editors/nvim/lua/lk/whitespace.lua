...
function M.setup()
  require('lk.highlights').highlight('ExtraWhitespace', { guifg = 'red' })
  require('lk.autocommands').augroup(
      'WhitespaceMatch', {
        {
          events = { 'ColorScheme' },
          targets = { '*' },
          command = [[lua require("lk.highlights").highlight("ExtraWhitespace", {guifg = "red"})]]
        },
        {
          events = { 'BufEnter', 'FileType', 'InsertLeave' },
          targets = { '*' },
          command = [[lua require('lk.whitespace').toggle_trailing('n')]]
        },
        {
          events = { 'InsertEnter' },
          targets = { '*' },
          command = [[lua require('lk.whitespace').toggle_trailing('i')]]
        }
      }
  )
end
