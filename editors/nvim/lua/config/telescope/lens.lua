    ...
    pickers.new(
        opts, {
          ...
          on_input_filter_cb = function(prompt)
            ...
            local new_result = vim.lsp.buf_request_sync(
                                   bufnr, 'workspace/symbol', params
                               )
            local new_locations = {}
            for _, server_results in pairs(new_result or {}) do
              if server_results.result then
                vim.list_extend(
                    new_locations, vim.lsp.util.symbols_to_items(
                        server_results.result, 0
                    ) or {}
                )
              end
            end
...
M.live_workspace_symbols = function(opts)
  ...
  opts.shorten_path = utils.get_default(opts.shorten_path, false)
  ...
  vim.lsp.buf_request(
      0, 'workspace/symbol', params, M._create_workspace_handler(opts)
  )
end
